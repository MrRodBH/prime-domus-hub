...
import { adminListarBairros, adminSalvarBairro, adminExcluirBairro } from "@/lib/api/admin.functions";
...
function novo(): Bairro { return { nome: "", slug: "", cidade: "Belo Horizonte", estado: "MG", descricao: "", imagem_url: "", destaque: false, ordem: 0 }; }
...
function AdminBairros() {
  ...
  const salvar = useMutation({
    ...
    onSuccess: () => { toast.success("Salvo"); qc.invalidateQueries({ queryKey: ["admin", "bairros"] }); setOpen(false); },
  ...
  return (
    <div className="space-y-6">
            ...
            {editing && (
              <form onSubmit={(e) => { e.preventDefault(); salvar.mutate(editing); }} className="space-y-3">
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div><Label>Estado</Label><Input value={editing.estado} onChange={(e) => setEditing({ ...editing, estado: e.target.value })} /></div>
      ...
      <div className="bg-card border border-foreground/5 rounded-lg overflow-hidden">
        <Table>
          <TableHeader><TableRow><TableHead>Nome</TableHead><TableHead>Cidade</TableHead><TableHead>Ordem</TableHead><TableHead>Destaque</TableHead><TableHead></TableHead></TableRow></TableHeader>
          <TableBody>
            {data?.map((b) => (
              <TableRow key={b.id}>
                <TableCell className="font-medium">{b.nome}</TableCell>
                <TableCell>{b.cidade}/{b.estado}</TableCell>
