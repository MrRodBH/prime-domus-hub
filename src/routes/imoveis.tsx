                ...
                return (
                  <Link to="/imovel/$slug" params={{ slug: p.slug }} key={p.id} className="group block" aria-label={p.titulo}>
                    <div className="relative overflow-hidden rounded mb-5 aspect-[4/5] bg-muted">
                      <img src={p.imagem_capa || imovelImage(p.slug, i)} alt={p.titulo} width={800} height={1000} loading="lazy" className="w-full h-full object-cover transition-transform duration-[800ms] group-hover:scale-[1.04]" />
                      {p.badge && (
