...
export const getTenantCmsFunctionalInventory = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    await authorizeTenantCmsOperation(context, "cms.paginas", "read");
    return {
      ...CMS_FUNCTIONAL_EXTENSION_CONTRACT,
...
export const validateTenantCmsFunctionalComponent = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }): Promise<{
    ...
    await authorizeTenantCmsOperation(
      context,
      definition.permissionContract.module,
      "save_draft",
    );
...
async function validateReferences(context: any, key: CmsFunctionalComponentKey, value: any) {
  ...
  if (key === "cards") {
    checks.push({
      table: "media_library",
      ids: value.cards.map((card: any) => card.mediaId).filter(Boolean),
    });
  }
  for (const check of checks) {
    ...
    if (error || (data ?? []).length !== ids.length) {
      throw new Error("cms_functional_cross_tenant_reference");
    }
  }
}
...
export const saveTenantCmsTestimonial = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    const auth = await authorizeTenantCmsOperation(context, "cms.paginas", "save_draft");
    const admin = await adminClient();
    if (data.mediaId) {
    ...
    if (data.id) {
      ...
      if (data.expectedVersion !== undefined && Number(rows[0].row_version) !== data.expectedVersion) {
      ...
      const { data: row, error: updateError } = await admin
        ...
        .eq("tenant_id", auth.tenantId)
        .eq("id", data.id)
        .eq("row_version", rows[0].row_version)
...
export const listTenantCmsReusableBlocks = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const { data, error } = await admin
      ...
      .select("id, block_key, name, revision, status, schema_version, content_hash, created_at, updated_at")
      .eq("tenant_id", auth.tenantId)
      .order("block_key")
...
export const saveTenantCmsReusableBlock = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    const operation = data.status === "published" ? "publish" : "save_draft";
    const auth = await authorizeTenantCmsOperation(context, "cms.paginas", operation);
    const content = JSON.stringify(data.content);
    if (content.length > 1_000_000 || /<script|javascript:|eval\(|new function/i.test(content)) {
      throw new Error("cms_runtime_code_prohibited");
    }
...
export const listTenantCmsPublicationSchedules = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const admin = await adminClient();
    const { data, error } = await admin
      .from("cms_publication_schedules")
      .select("id, page_id, version_id, revision, publish_at, timezone, state, created_at, updated_at")
      .eq("tenant_id", auth.tenantId)
...
export const scheduleTenantCmsPublication = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    const auth = await authorizeTenantCmsOperation(context, "cms.paginas", "publish");
    const admin = await adminClient();
...
export const cancelTenantCmsPublicationSchedule = createServerFn({ method: "POST" })
  .middleware([requireTenant])
  .inputValidator((input: unknown) =>
    z.object({ scheduleId: uuid }).strict().parse(input),
  )
  .handler(async ({ context, data }) => {
    const auth = await authorizeTenantCmsOperation(context, "cms.paginas", "publish");
    const admin = await adminClient();
