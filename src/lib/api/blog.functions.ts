...
export const listarCategoriasPublicas = createServerFn({ method: "GET" }).handler(async () => {
  ...
  if (error) throw new Error(error.message);
  return assertTenantScopedRows(tenant.id, data as unknown as PublicBlogRow[] | null).map(withoutTenantId);
});

async function authorizeBlog(context: Parameters<typeof authorizeTenantCmsOperation>[0], operation: "list" | "read" | "create_draft" | "save_draft" | "publish" | "delete") {
  return authorizeTenantCmsOperation(context, "cms.paginas", operation);
}

async function requireTenantReference(context: any, tenantId: string, table: "blog_categorias" | "corretores", id: string | null | undefined) {
  if (!id) return null;
  const { data, error } = await context.supabase.from(table).select("id").eq("tenant_id", tenantId).eq("id", id).limit(2);
...
export const adminSalvarCategoria = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (id) {
      const { data: updated, error } = await context.supabase.from("blog_categorias").update(fields as never)
        .eq("tenant_id", authorization.tenantId).eq("id", id).select("id").maybeSingle();
      if (error) throw safeTenantCmsError(error);
      if (!updated) throw new Error("cms_cross_tenant_reference");
      return { ok: true, id };
    }
    const { data: inserted, error } = await context.supabase.from("blog_categorias")
...
export const adminExcluirCategoria = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: deleted, error } = await context.supabase.from("blog_categorias").delete()
...
export const adminListarPosts = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    return (data ?? []).map((row) => {
      ...
      if (categoria && categoria.tenant_id !== authorization.tenantId) throw new Error("cms_cross_tenant_reference");
...
export const adminObterPost = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: rows, error } = await context.supabase.from("blog_posts").select("*")
...
export const adminSalvarPost = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const operation = data.status === "publicado" ? "publish" : data.id ? "save_draft" : "create_draft";
    ...
    const { id, ...fields } = data;
    const payload = { ...fields, publicado_em: data.status === "publicado" ? new Date().toISOString() : null };
    if (id) {
      const { data: updated, error } = await context.supabase.from("blog_posts").update(payload as never)
        .eq("tenant_id", authorization.tenantId).eq("id", id).select("id").maybeSingle();
      if (error) throw safeTenantCmsError(error);
      if (!updated) throw new Error("cms_cross_tenant_reference");
      return { ok: true, id };
    }
    const { data: inserted, error } = await context.supabase.from("blog_posts")
      .insert({ ...payload, tenant_id: authorization.tenantId } as never).select("id").single();
    if (error) throw safeTenantCmsError(error);
    return { ok: true, id: inserted.id };
  });
...
export const adminExcluirPost = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const authorization = await authorizeBlog(context, "delete");
    const { data: deleted, error } = await context.supabase.from("blog_posts").delete()
      .eq("tenant_id", authorization.tenantId).eq("id", data.id).select("id").maybeSingle();
    if (error) throw safeTenantCmsError(error);
    if (!deleted) throw new Error("cms_cross_tenant_reference");
    return { ok: true };
...
const aiInput = z.object({ conteudo: z.string().min(20), titulo: z.string().optional().default("") }).strict();
...
export const adminImportarPdf = createServerFn({ method: "POST" })
  .middleware([requireTenant])
  .inputValidator((input: unknown) => z.object({ pdfBase64: z.string().min(100), nomeArquivo: z.string().max(300).optional().default("") }).strict().parse(input))
  .handler(async ({ context }) => {
