...
export const MARKETING_CHANNEL_KEYS = ["META_ADS", "GOOGLE_ADS", "MANUAL_IMPORT", "WEBSITE_FORM"] as const;
...
export const MARKETING_INGESTION_STATES = [
  "received", "verification_failed", "verified", "mapping_failed", "normalized",
  "duplicate_detected", "lead_created", "lead_linked", "rejected", "retryable_failed", "terminal_failed",
] as const;
export const MARKETING_IMPORT_STATES = ["draft", "preview_ready", "processing", "partial_success", "completed", "failed"] as const;
export const MARKETING_IMPORT_ROW_STATES = ["received", "valid", "invalid", "duplicate_detected", "lead_created", "failed"] as const;
export const MARKETING_MANUAL_METHODS = ["CSV", "XLSX", "MANUAL_ROW"] as const;
export const MARKETING_AUTOMATED_METHODS = ["LEAD_AD_WEBHOOK", "PTW01_HOST_DERIVED"] as const;
...
export type MarketingChannelDefinition = {
  ...
  schemaVersion: 1;
  adapterImplementationState: "implemented" | "not_applicable";
  externalVerificationState: "not_live_verified" | "not_applicable";
  externalExecutionState: "not_executed";
  ...
  providerVerificationContract: "meta_x_hub_signature_256" | "google_webhook_key" | "host_derived" | "not_applicable";
...
export const MARKETING_CHANNEL_REGISTRY = [
  {
    ...
    adapterImplementationState: "implemented",
    externalVerificationState: "not_live_verified",
    manualMethods,
    ...
    providerVerificationContract: "meta_x_hub_signature_256",
    leadWriterContract: "canonical_crm_internal",
    availabilityState: "credential_required",
  },
  {
    ...base,
    ...
    credentialContract: "reference_only_no_inline_secret",
    signatureContract: "provider_adapter_required",
    providerVerificationContract: "google_webhook_key",
    leadWriterContract: "canonical_crm_internal",
    availabilityState: "credential_required",
  },
  {
    ...
    adapterImplementationState: "not_applicable",
    externalVerificationState: "not_applicable",
    manualMethods,
    automatedMethods: [],
    credentialContract: "not_required",
    signatureContract: "not_applicable",
    providerVerificationContract: "not_applicable",
  ...
  },
  {
    ...
    providerVerificationContract: "host_derived",
...
const inlineSecretTokens = [
  ...
  "private_key", "refresh_token", "client_secret", "access_token", "app_secret",
];
export function assertNoMarketingInlineSecrets(value: unknown, path = "config"): void {
  ...
  for (const [key, child] of Object.entries(value as Record<string, unknown>)) {
    ...
    if (
      key !== "credentialReference" && key !== "credential_reference" &&
...
export const MarketingFieldMappingSchema = z.object({
  name: sourcePath,
  email: sourcePath.nullable().default(null), phone: sourcePath.nullable().default(null), message: sourcePath.nullable().default(null),
  property_reference: sourcePath.nullable().default(null), source: sourcePath.nullable().default(null),
  campaign_id: sourcePath.nullable().default(null), campaign_name: sourcePath.nullable().default(null),
  adset_id: sourcePath.nullable().default(null), adset_name: sourcePath.nullable().default(null),
  ad_id: sourcePath.nullable().default(null), ad_name: sourcePath.nullable().default(null),
  utm_source: sourcePath.nullable().default(null), utm_medium: sourcePath.nullable().default(null),
  utm_campaign: sourcePath.nullable().default(null), utm_content: sourcePath.nullable().default(null), utm_term: sourcePath.nullable().default(null),
  gclid: sourcePath.nullable().default(null), fbclid: sourcePath.nullable().default(null),
  landing_url: sourcePath.nullable().default(null), referrer: sourcePath.nullable().default(null), provider_payload_id: sourcePath.nullable().default(null),
}).strict();
...
export const MarketingAttributionSchema = z.object({
  ...
  campaignId: z.string().max(200).nullable(), campaignName: z.string().max(300).nullable(),
  adsetId: z.string().max(200).nullable(), adsetName: z.string().max(300).nullable(),
  adId: z.string().max(200).nullable(), adName: z.string().max(300).nullable(),
  utmSource: z.string().max(300).nullable(), utmMedium: z.string().max(300).nullable(), utmCampaign: z.string().max(300).nullable(),
  utmContent: z.string().max(500).nullable(), utmTerm: z.string().max(500).nullable(),
  gclid: z.string().max(500).nullable(), fbclid: z.string().max(500).nullable(),
  landingUrl: z.string().max(2000).nullable(), referrer: z.string().max(2000).nullable(),
  providerPayloadId: z.string().max(300).nullable(), connectorId: z.string().uuid(),
  mappingVersion: z.number().int().min(1), receivedAt: z.string().datetime(),
}).strict();
