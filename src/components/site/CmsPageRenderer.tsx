...
type GalleryItem = Extract<CmsBlock, { type: "gallery" }>['data']['imagens'][number];

function hasPublicGalleryUrl(item: GalleryItem): item is GalleryItem & { url: string } {
...
function BlockRenderer({ block }: { block: CmsBlock }) {
  switch (block.type) {
    ...
    case "gallery": {
      ...
      const images = d.imagens.filter(hasPublicGalleryUrl);
      return (
        <section className="max-w-6xl mx-auto px-4 py-8">
          <div className={`grid ${cols} gap-3`}>
            {images.map((image, index) => (
              <img key={`${image.url}:${index}`} src={image.url} alt={image.alt ?? ""} loading="lazy" className="w-full h-64 object-cover rounded" />
            ))}
          </div>
        </section>
      );
        ...
        <section className="max-w-6xl mx-auto px-4 py-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {d.itens.map((item, index) => (
              <div key={`${item.titulo}:${index}`} className="p-6 rounded-lg border bg-card">
                {item.icone && <div className="text-3xl mb-3">{item.icone}</div>}
      ...
      return (
        <section className="max-w-3xl mx-auto px-4 py-12">
          ...
          <div className="space-y-4">
            {d.itens.map((item, index) => (
              <details key={`${item.pergunta}:${index}`} className="group p-4 rounded-lg border bg-card">
                <summary className="cursor-pointer font-medium">{item.pergunta}</summary>
                <p className="mt-3 text-muted-foreground text-sm">{item.resposta}</p>
              </details>
            ))}
          </div>
        </section>
