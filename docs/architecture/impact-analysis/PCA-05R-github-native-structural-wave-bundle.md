...
Compatibilidade PostgreSQL 17: a migration `20260728180000` mantém nos próprios bytes-fonte quatro casts explícitos de `pg_attribute.attname` para `text` antes da comparação com `text[]`. O bundle valida esses quatro casts e rejeita qualquer retorno da forma incompatível `name[] = text[]`; nenhuma projeção transitória é necessária.
