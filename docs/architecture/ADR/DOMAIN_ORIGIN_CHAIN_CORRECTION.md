...
This changes where tenant websites are published. The owner's 2026-09-14 instruction selects this alternative subject to verification and explicitly requires validation before traffic cutover. It does not move the canonical backend or realone.com.br platform hosting. The existing code already has the Worker runtime/bootstrap/build contract; this is not proof that its present real deployment is ready. See [OWNED_ORIGIN_PREPARATION.md](../../operations/OWNED_ORIGIN_PREPARATION.md) for measured evidence, the precise external custody configuration, and remaining acceptance criteria.
...
2. Backend/Cloudflare operator: provide server-only runtime bindings through a secure configuration channel: SUPABASE_URL, SUPABASE_PUBLISHABLE_KEY, SUPABASE_SERVICE_ROLE_KEY and the SAME DOMAIN_ROUTING_PROOF_SECRET used by the Edge processor. The canonical project ref is stmcnvzuzlyqammyycxj and URL https://stmcnvzuzlyqammyycxj.supabase.co. Build-time public configuration uses VITE_SUPABASE_URL and VITE_SUPABASE_PUBLISHABLE_KEY. Never copy service-role or proof keys into VITE variables, chat or source. The managed service-role value is not available to the owner; do not ask the owner to retrieve it. The managed runtime can read it internally. A Cloudflare provisioning credential reachable from that runtime is the current external prerequisite for a reviewed server-to-server transfer; the transfer is not yet implemented or executed. Inventory remaining feature-specific runtime bindings from canonical bootstrap before deployment; do not assume this list proves complete application readiness. Connector Cloudflare access is not runtime Supabase credential provisioning.
