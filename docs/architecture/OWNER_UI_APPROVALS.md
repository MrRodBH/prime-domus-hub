...
## Owner clarification — independent company / first Admin setup (2026-09-10)
A company can originate from a sales platform or direct sale registered by Super Admin. Company registration is the first setup milestone; registering and activating its first operational Admin is a separate, subsequent step. Neither an existing tenant Admin nor another Super Admin account is a prerequisite.
This authorizes only initial setup for an operational customer without an operational owner/Admin. It does not authorize general tenant-user CRUD or tenant impersonation by Super Admin. Existing owner UUID/references remain unchanged; new company registration does not guess ownership or create an Auth identity. First-Admin activation grants the non-owner tenant Admin membership plus the existing system Admin RBAC profile scoped to that tenant, after verified-email acceptance. This is the foundational access milestone, analogous to initial owner bootstrap; ordinary member invitations keep their existing commercial seat enforcement and no entitlements are changed.
Real names/emails are entered by the owner through the interface. Auth email delivery and activation are different states. A confirmed existing account accepts after its own login; it is never reset, recreated or reassigned. Sales origin/reference records are provenance, not evidence of paid checkout or automatic billing integration. No roadmap changes.
...
## Owner corrective — tenant user administration (2026-09-13)
The owner approves the interface and clarifies that the platform Super Admin must never appear in the tenant's operational directory. Preserve historical ownership references without displaying them as tenant operators; do not transfer ownership or delete existing identities. Tenant Admin creates a new user with name, email, role and configured access profiles, then sends activation so the recipient sets their own password. System profiles remain immutable references with an explicit tenant-owned customizable copy. This supersedes invitation-first copy and the Round65 exact-file freeze. Preserve the approved platform and tenant themes. No real accounts, memberships or activation emails are created by automated verification.
