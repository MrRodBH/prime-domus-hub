...
## Managed packager correction (2026-09-14)

The real native deployer rejected the reviewed bundle with Module not found because it excludes ignored files. The canonical generated sibling is therefore tracked, with a CI equality check against a fresh deterministic build. This supersedes the earlier exclusion rule: every processor source change requires node scripts/domains/build-edge.mjs and inclusion of the resulting artifact in its PR. Deploy the tracked index and generated sibling together, with no temporary .gitignore edits or Lovable Git operations. Domain authority, scheduling and secrets remain unchanged. The native deployment of the corrected artifact remains required; a successful local build is not a successful live deployment.
