...
The same live permission check found zero system-Admin permissions for the canonical crm module. The tenant-owned initial operational profile additionally grants exactly visualizar/criar/editar/gerenciar used by the current CRM operation map, without changing any global profile or other tenant assignment. The existing first Admin therefore receives CMS and CRM authority plus access management, while ownership remains separate.
