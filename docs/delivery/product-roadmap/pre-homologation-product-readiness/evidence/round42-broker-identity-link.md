...
The initial PR head passed the Round 42 and Round 40 CI gates. Both protected composite gates stopped on the same exact-inventory assertion: the newly added migration was not classified in the PCA-05R prerequisite manifest. The consolidated correction explicitly appends it as `POST_REHEARSAL_ROUND42_REPOSITORY_ONLY_IDENTITY_LINK / EXCLUDED_FROM_PCA05R_CELL`, with its SHA-256, and evolves the exact inventory counts from 131/9 to 132/10 (total/excluded). The 105 prerequisites, 17 rehearsal migrations, their digests, historical source references, decisions and prohibited replay operations remain unchanged. The focused diff check permits precisely this new migration for the Round 42 test change; unclassified migrations still fail. The subsequent composite execution exposed cumulative historical scope assertions in PCA-12B, PCA-12C-R3 and PCA-15R; these receive the twelve explicit Round 42 repository paths, including the regression runners themselves. Their historical content, authority, secret-custody and frozen-file assertions remain active. The PCA-15R cumulative migration assertion now admits exactly the new Round 42 migration when this successor is present; all other migrations remain forbidden there. This is maintenance of downstream regression scope, not reopening or executing the historical gates operationally. Full composite validation is performed by the protected CI runners; the local tsx CLI cannot create its IPC socket in this workspace.
