...
- Testes unitários: 43 casos em `membership-mutation-input.spec.ts`, `run-tenant-specs.ts` → 219 passed, 0 failed, typecheck exit 0.
- Integração crítica: 14 cenários reais de integração em `run-membership-mutation-parity-specs.ts`, com fixtures isoladas, cleanup fail-closed e zero fixtures residuais. ACL da função comprovada separadamente pela migration fail-closed `20260713214913_f49f898c-...sql` e pelas consultas SQL diretas a `pg_proc`, `aclexplode` e `has_function_privilege` executadas durante a implementação. Probes runtime separados para `anon`, `authenticated` e `service_role` complementam a evidência.
...
Authorized next step; not started.
...
SCP-012.0.3 está Accepted.

SCP-012 — Commercial Seat Limit Atomic Enforcement Integration está autorizada como próxima etapa e ainda não foi iniciada.
