...
for (const publicPath of ["/", "/imoveis", "/imovel/casa-a", "/blog/", "/p/pagina", "/sitemap.xml"]) {
  assert(requiresPublicTenantPreflight(publicPath), `${publicPath} must require public tenant preflight`);
}
for (const excludedPath of ["/admin", "/admin/leads", "/super", "/auth", "/reset-password", "/api/public/portal-leads", "/lovable/email/queue/process", "/email/unsubscribe"]) {
