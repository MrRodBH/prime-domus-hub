...
assert.equal((source.match(/console\.error\(/g) ?? []).length, 1);
...
for (const [event, metadata] of captured) {
  assert.equal(event, "email_unsubscribe_error");
  assert.deepEqual(Object.keys(metadata as Record<string, unknown>).sort(), [
    "correlation",
    "error_class",
    "event_code",
    "route",
