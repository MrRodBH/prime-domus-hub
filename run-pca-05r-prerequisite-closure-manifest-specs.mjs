...
const excludedAfterPrelude = {
  "20260914210012_7fdd3240-0b97-4fdd-8c5e-9cc92af6d9ba.sql": "POST_REHEARSAL_CMS_DIGEST_SCHEMA_CORRECTION",
...
if (base) {
  ...
  } else if (changed.includes("supabase/migrations/20260914210012_7fdd3240-0b97-4fdd-8c5e-9cc92af6d9ba.sql")) {
    const path = "supabase/migrations/20260914210012_7fdd3240-0b97-4fdd-8c5e-9cc92af6d9ba.sql";
    assert.deepEqual(changedMigrations, [path]);
    assert.equal(actual.excluded.find(entry => entry.path === path)?.reason, "POST_REHEARSAL_CMS_DIGEST_SCHEMA_CORRECTION");
    assert.equal(actual.rehearsal.some(entry => entry.path === path), false);
  } else {
