...
assert.deepEqual(Object.values(manifest.waves).map((wave) => wave.sha256), [
  "39a92ce7d2e11c56a43be5065b19ddf18c7c934c16125810caef34a44c5c07d8",
...
assert.equal(manifest.entries.filter((entry) => entry.projection === "SOURCE_EXACT_PG17_NAME_ARRAY_TO_TEXT_ARRAY").length, 1);
assert.equal((outputs.W1.sql.match(/array_agg\(a\.attname::text ORDER BY x\.ord\)/g) ?? []).length, 4);
assert.doesNotMatch(outputs.W1.sql, /array_agg\(a\.attname ORDER BY x\.ord\)/);
