...
BEGIN
  ...
  -- Lock and validate the server-issued target before any commercial/CMS decision.
  IF _cover_upload_target_id IS NOT NULL THEN
    SELECT * INTO v_target
      FROM public.tenant_upload_targets
     WHERE tenant_id = _tenant_id
       AND id = _cover_upload_target_id
     FOR UPDATE;
    IF NOT FOUND THEN
    ...
    IF v_target.domain <> 'blog-cover'
       ...
       OR v_target.entity_id IS NOT NULL THEN
    ...
    END IF;
    v_required_prefix := _tenant_id::text || '/blog/';
    IF left(v_target.path, length(v_required_prefix)) IS DISTINCT FROM v_required_prefix THEN
      RAISE EXCEPTION 'blog_cover_path_mismatch' USING ERRCODE = '42501';
    END IF;
    IF NOT EXISTS (
      SELECT 1
        FROM storage.objects
       WHERE bucket_id = v_target.bucket
         AND name = v_target.path
    ) THEN
  ...
  END IF;
  v_cover_path := CASE
    WHEN _cover_upload_target_id IS NOT NULL THEN v_target.path
    ELSE v_existing_cover
  END;
  ...
  IF _post_id IS NULL THEN
    INSERT INTO public.blog_posts (
      ...
      _category_id,
      _author_id,
      _status::public.blog_post_status,
      CASE WHEN _status = 'publicado' THEN now() ELSE NULL END,
      NULLIF(_meta_title, ''),
      NULLIF(_meta_description, '')
  ...
  ELSE
    UPDATE public.blog_posts
       SET titulo = trim(_title),
           ...
           publicado_em = CASE
             WHEN _status = 'publicado' THEN COALESCE(publicado_em, now())
             ELSE NULL
           END,
...
BEGIN
  ...
  END IF;
  IF length(COALESCE(_title, '')) > 300
     OR length(COALESCE(_legend, '')) > 500
     OR _order < 0
     OR _order > 10000 THEN
    RAISE EXCEPTION 'launch_upload_invalid_metadata' USING ERRCODE = '22023';
  END IF;

  -- Target lock precedes permission and project decisions.
  SELECT * INTO v_target
    FROM public.tenant_upload_targets
   WHERE tenant_id = _tenant_id
     AND id = _target_id
  ...
  v_required_prefix := CASE _operation
    WHEN 'cover' THEN _tenant_id::text || '/' || v_slug || '/capa/'
    WHEN 'gallery' THEN _tenant_id::text || '/' || v_slug || '/galeria/'
    ELSE _tenant_id::text || '/' || v_slug || '/' || COALESCE(_kind, '') || '/'
  END;
  IF _operation = 'pdf' AND _kind NOT IN ('tabela_precos','manual') THEN
    RAISE EXCEPTION 'launch_pdf_kind_invalid' USING ERRCODE = '22023';
  END IF;
  IF left(v_target.path, length(v_required_prefix)) IS DISTINCT FROM v_required_prefix THEN
    RAISE EXCEPTION 'launch_upload_path_mismatch' USING ERRCODE = '42501';
  END IF;
  IF _operation = 'pdf'
     AND _kind = 'tabela_precos'
     AND (
       SELECT count(*)
         FROM public.launch_pdfs
        WHERE tenant_id = _tenant_id
          AND project_id = _project_id
          AND kind = 'tabela_precos'
     ) >= 3 THEN
  ...
  IF v_resource_id IS NULL THEN
    RAISE EXCEPTION 'launch_media_persistence_failed' USING ERRCODE = 'P0001';
  END IF;
  ...
  VALUES (
    _tenant_id,
    _actor_user_id,
    'launch.media.upload_target_consumed',
    CASE _operation
      WHEN 'cover' THEN 'launch_projects'
      WHEN 'gallery' THEN 'launch_project_imagens'
      ELSE 'launch_pdfs'
    END,
    v_resource_id::text,
    jsonb_build_object(
