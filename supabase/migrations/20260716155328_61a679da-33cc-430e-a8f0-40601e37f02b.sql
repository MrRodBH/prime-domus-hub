...
-- LSH-01 · Lote B — SQL Authority Alignment
-- Additive migration. Only reshapes public.create_manual_lead to match the
-- canonical runtime contract (is_super_admin + header-derived impersonation +
-- explicit cardinality). Audit table hardening from Lote A is preserved.
...
BEGIN
  ...
  -- 3) Canonical Super Admin detection + independent impersonation evidence.
  ...
  EXCEPTION WHEN OTHERS THEN
    v_header_raw := NULL;
  END;
  -- Impersonation evidence is the presence of a non-empty x-tenant-id header
  -- explicitly declared by the client transport. This mirrors the canonical
  -- resolution used by public.get_current_tenant_id.
  v_is_impersonating := (v_header_raw IS NOT NULL AND v_header_raw <> '');
  ...
  -- 4) Tenant resolution (canonical function; no client-supplied argument).
  v_tenant := public.get_current_tenant_id();
  IF v_tenant IS NULL THEN
    RAISE EXCEPTION 'tenant_not_resolved' USING ERRCODE = '42501';
  END IF;

  -- 5) Membership cardinality — regular users only.
  ...
  ELSE
    ...
    -- 6) Functional authorization (admin | corretor).
...
-- Reassert explicit RPC grants (Lote B keeps the Lote A posture explicitly).
