...
let allowed=false;let calls=[];let providerError=false;
...
globalThis.__admin={auth:{admin:{getUserById:async uid=>{calls.push(['get',uid]);return {data:{user:{id:uid,email:'fixture@example.invalid'}}}},deleteUser:async(...args)=>{calls.push(['delete',...args]);return {error:providerError?{code:'dependency'}:null}}}}};
...
console.log('PASS global account deletion: server role, strict input, exact account confirmation, permanent deletion, no role exception, provider failure preserved.');
