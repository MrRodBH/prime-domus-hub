...
function rejects(label: string, action: () => unknown, token: string) {
  try {
    ...
    check(
      ...
      `${label} must reject with ${token}`,
    );
  }
}

check(
  PORTAL_CONNECTOR_REGISTRY.length === 4,
  "closed registry must contain four automated transport contracts",
);
check(
  new Set(PORTAL_CONNECTOR_REGISTRY.map((item) => item.connectorKey)).size === 4,
  "connector keys must be unique",
);
check(
  PORTAL_CONNECTOR_REGISTRY.every((item) => item.operationMode === "HYBRID"),
  "all connector definitions must be HYBRID",
);
check(
  PORTAL_CONNECTOR_REGISTRY.every((item) => item.availabilityState === "adapter_not_implemented"),
  "automated adapters must fail closed",
);
check(
  PORTAL_CONNECTOR_REGISTRY.every(
    (item) =>
      item.manualMethods.includes("CSV") &&
      item.manualMethods.includes("XLSX") &&
      item.manualMethods.includes("MANUAL_EXPORT"),
  ),
  "every connector must expose all manual methods",
);
check(
  PORTAL_AUTOMATED_METHODS.every(
    (method) => getPortalConnectorDefinition(method).connectorKey === method,
  ),
  "every automated method must be cataloged",
);
check(
  PORTAL_CONFIGURATION_STATES.includes("credential_provisioning_required"),
  "credential provisioning state must exist",
);
check(
  PORTAL_CONFIGURATION_STATES.includes("adapter_not_implemented"),
  "adapter-not-implemented state must exist",
);
...
rejects(
  "unknown connector",
  () => getPortalConnectorDefinition("INVENTED"),
  "portal_connector_not_cataloged",
);
...
rejects(
  "inline token",
  () => assertNoPortalInlineSecrets({ api_token: "raw" }),
  "portal_inline_secret_prohibited",
);
rejects(
  "inline password",
  () => assertNoPortalInlineSecrets({ nested: { password: "raw" } }),
  "portal_inline_secret_prohibited",
);
rejects(
  "wrong operation mode",
  () => parsePortalHybridConfig({ ...config, operation_mode: "AUTOMATED" }),
  "",
);
rejects("unknown config key", () => parsePortalHybridConfig({ ...config, arbitrary: true }), "");
rejects(
  "bad credential reference",
  () => parsePortalHybridConfig({ ...config, credential_reference: "raw-secret" }),
  "",
);
...
rejects(
  "invalid job jump",
  () => assertPortalJobTransition("queued", "published"),
  "portal_job_transition_invalid",
);
rejects(
  "terminal retry",
  () => assertPortalJobTransition("failed_terminal", "retry_scheduled"),
  "portal_job_transition_invalid",
);
rejects(
  "cancelled mutation",
  () => assertPortalJobTransition("cancelled", "queued"),
  "portal_job_transition_invalid",
);

check(
  validatePortalEndpoint("https://portal.example.com/api")?.startsWith("https://") === true,
  "HTTPS endpoint must be accepted",
);
rejects(
  "HTTP endpoint",
  () => validatePortalEndpoint("http://portal.example.com"),
  "portal_endpoint_https_required",
);
rejects(
  "private endpoint",
  () => validatePortalEndpoint("https://192.168.1.4/api"),
  "portal_endpoint_private_network_prohibited",
);
rejects(
  "embedded endpoint credentials",
  () => validatePortalEndpoint("https://user:pass@portal.example.com"),
  "portal_endpoint_credentials_prohibited",
);
...
check(
  payload.media[0]?.persistedReference === media[0].url,
  "payload must use persisted media reference",
);
rejects(
  "cross tenant media",
  () =>
    buildPortalPublicationPayload({
      property,
      media: [{ ...media[0], tenant_id: "44444444-4444-4444-8444-444444444444" }],
      mapping: DEFAULT_PORTAL_MAPPING,
    }),
  "tenant_portal_cross_tenant_media",
);
rejects(
  "draft property",
  () =>
    buildPortalPublicationPayload({
      property: { ...property, status: "rascunho", publicado_em: null },
      media: [],
      mapping: DEFAULT_PORTAL_MAPPING,
    }),
  "tenant_portal_property_ineligible",
);
...
check(
  payloadHashA === payloadHashB && /^[0-9a-f]{64}$/.test(payloadHashA),
  "payload hash must be deterministic SHA-256",
);
const adapterResult = await getPortalAdapter("JSON_API").publish({
  endpoint: null,
...
check(
  !adapterResult.ok && adapterResult.errorCode === "adapter_not_implemented",
  "automated adapter must never fake success",
);
...
check(
  !/\.from\("portal_connectors"\)[\s\S]{0,500}\.(insert|update|upsert|delete)/.test(functions),
  "server functions must not mutate connector tables directly",
);
check(
  !/\.from\("tenant_portal_jobs"\)[\s\S]{0,500}\.(insert|update|upsert|delete)/.test(functions),
  "server functions must not mutate jobs directly",
);
...
check(
  !/order\([\s\S]{0,100}limit\(1\)/i.test(worker),
  "worker must not select authoritative job through ORDER BY/LIMIT 1",
);
...
check(
  !/ORDER BY[\s\S]{0,100}LIMIT\s+1/i.test(migration),
  "migration must not use ORDER BY/LIMIT 1 as authority",
);
check(
  !/GRANT\s+(SELECT|INSERT|UPDATE|DELETE|ALL)[\s\S]{0,100}\sTO\s+(anon|authenticated)/i.test(
    migration,
  ),
  "migration must not grant direct portal table access",
);
check(
  !migration.includes("feed_token = NULL") && !migration.includes("webhook_secret = NULL"),
...
check(
  legacyBarrel.includes("Read-only compatibility aliases"),
  "legacy module must be a single canonical barrel",
);
...
includesAll(
  ...
  [
    ...
    "XLSX",
    "MANUAL_EXPORT",
    "Somente a referência — não informe a credencial",
    "Salvar não ativa a automação",
    "Nenhuma publicação real executada",
  ],
  "portal UI",
);
