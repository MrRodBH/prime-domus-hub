...
const throws = (fn: () => unknown, token: string, message = token): void => {
  assert.throws(fn, (error: unknown) => error instanceof Error && error.message.includes(token), message);
  assertions += 1;
};
const has = (source: string, token: string, message = token): void => {
  ok(source.includes(token), `missing ${message}`);
};
const lacks = (source: string, token: string, message = token): void => {
...
// Closed build-time registries and factual provider availability.
...
for (const state of ["manual_ready", "automated_ready", "adapter_not_implemented", "mapping_required", "failed"]) {
  ok(MARKETING_AVAILABILITY_STATES.includes(state as never), `${state}:availability state`);
}
...
// Strict connector config and secret rejection.
const manualConfig = {
...
equal(MarketingConnectorConfigSchema.safeParse({ ...manualConfig, tenantId: "x" }).success, false, "tenant input denied");
equal(MarketingConnectorConfigSchema.safeParse({ ...manualConfig, actorUserId: "x" }).success, false, "actor input denied");
...
// Closed mapping targets and no authority fields.
...
for (const forbidden of ["tenant_id", "tenantId", "actor_user_id", "actorUserId", "assigned_to", "pipeline_id", "stage_id"]) {
  equal(MarketingFieldMappingSchema.safeParse({ ...DEFAULT_MARKETING_FIELD_MAPPING, [forbidden]: forbidden }).success, false, `${forbidden}:mapping authority denied`);
}
...
// Deterministic payload identity, sanitization and signature primitive.
...
equal(sanitized.nested.safe, "ok", "safe value preserved");
...
// Lead normalization and campaign/ad/UTM attribution.
...
for (const [actual, expected, label] of [
  [mapped.name, "Maria Silva", "name"], [mapped.normalizedEmail, "maria@example.com", "normalized email"],
  [mapped.normalizedPhone, "5531999999999", "normalized phone"], [mapped.propertyReference, "ABC-123", "property"],
  [mapped.attribution.channel, "META_ADS", "channel"], [mapped.attribution.provider, "meta", "provider"],
  [mapped.attribution.campaignId, "cmp-1", "campaign ID"], [mapped.attribution.campaignName, "Campanha 1", "campaign name"],
  [mapped.attribution.adsetId, "set-1", "adset ID"], [mapped.attribution.adsetName, "Público 1", "adset name"],
  [mapped.attribution.adId, "ad-1", "ad ID"], [mapped.attribution.adName, "Criativo 1", "ad name"],
  [mapped.attribution.utmSource, "facebook", "UTM source"], [mapped.attribution.utmMedium, "paid_social", "UTM medium"],
  [mapped.attribution.utmCampaign, "campanha-1", "UTM campaign"], [mapped.attribution.utmContent, "criativo-1", "UTM content"],
  [mapped.attribution.utmTerm, "imoveis", "UTM term"], [mapped.attribution.gclid, "gclid-data", "gclid"],
  [mapped.attribution.fbclid, "fbclid-data", "fbclid"], [mapped.attribution.providerPayloadId, "payload-1", "payload ID"],
...
// CSV/XLSX dry-run parsing and formula safety.
...
XLSX.utils.book_append_sheet(workbook, XLSX.utils.json_to_sheet([{ name: "Maria", email: "maria@example.com", phone: "31999999999" }]), "Leads");
...
equal(xlsxRows[0].name, "Maria", "XLSX name");
...
// Canonical tenant authority and server wrappers.
for (const token of [
...
for (const token of ['.rpc("has_role"', '.from("user_roles")', "ORDER BY", "LIMIT 1", "access_token:"]) lacks(files.authority, token);
for (const token of [
  ...
  "previewTenantMarketingManualImport", "createTenantMarketingManualImport", "executeTenantMarketingManualImport",
  "listTenantMarketingManualImports", "getTenantMarketingManualImport", "listTenantMarketingIngestionEvents",
  "getTenantMarketingIngestionEvent", "retryTenantMarketingIngestion", "getTenantMarketingDiagnostics",
  "receiveMarketingProviderPayload", "verifyMarketingProviderPayload", "ingestVerifiedMarketingLead",
]) has(files.functions, token);
for (const token of [
  ".inputValidator(z.object({ tenantId", ".inputValidator(z.object({ actorUserId",
  "_tenant_id: data.", "_actor_user_id: data.", "assigned_to: data.",
  '.from("leads").insert', '.from("leads").update', '.rpc("has_role"', '.from("user_roles")',
  "META_GRAPH_URL", "GOOGLE_ADS_URL", "fetch(", "@ts-ignore", "@ts-nocheck",
]) lacks(files.functions, token);

// Schema, provenance, OCC/idempotency, CRM insertion and ACL.
for (const token of [
  ...
  "assert_tenant_marketing_authority", "save_tenant_marketing_connector", "publish_tenant_marketing_connector",
  "set_tenant_marketing_credential_reference", "save_tenant_marketing_mapping",
  ...
  "retry_tenant_marketing_ingestion", "resolve_tenant_permission", "'crm'", "FOR UPDATE", "FOR SHARE",
  ...
  "landing_url", "referrer", "INSERT INTO public.audit_log", "ENABLE ROW LEVEL SECURITY",
  "FROM PUBLIC, anon, authenticated", "TO service_role",
]) has(files.migration, token);
for (const token of [
  "net.http", "http_post",
  "GRANT EXECUTE ON FUNCTION public.reserve_marketing_ingestion_payload(uuid,text,text,jsonb,integer) TO authenticated",
  "GRANT EXECUTE ON FUNCTION public.execute_tenant_marketing_manual_import(uuid,uuid,text,uuid,bigint) TO authenticated",
  "ORDER BY created_at LIMIT 1", "ORDER BY id LIMIT 1", "automatic_merge", "fuzzy",
]) lacks(files.migration, token);

// Functional UI and seven-context preservation.
for (const token of [
...
equal((files.contexts.match(/\n    id: "/g) ?? []).length, 7, "seven workspace contexts preserved");

// PTW-01 single public writer and deterministic CRM initial pipeline remain intact.
...
for (const token of ["public_marketing_lead_writer", "GRANT EXECUTE ON FUNCTION public.create_tenant_crm_lead"]) lacks(files.migration, token);
...
ok(assertions >= 220, `expected >= 220 assertions, got ${assertions}`);
