...
assert.equal(execFileSync('git',['diff','033a80eb47e3704183420de1453f309441dd62a6','--','src/lib/api','src/integrations','supabase','bun.lock','package.json','tests/round43',':(exclude)src/lib/api/super-onboarding.functions.ts', ':(exclude)src/integrations/supabase/__tests__/tenant-middleware.spec.ts', ':(exclude)src/integrations/supabase/tenant-middleware.ts', ':(exclude)src/lib/api/operational-tenants.server.ts', ':(exclude)src/lib/api/super-control-plane.functions.ts', ':(exclude)src/lib/api/site.functions.ts', ':(exclude)src/lib/api/super.functions.ts', ':(exclude)src/lib/api/tenant-scoped-authority.ts', ':(exclude)src/lib/api/tenant-crm.functions.ts', ':(exclude)src/lib/api/tenant-lifecycle.functions.ts',':(exclude)supabase/migrations/20260908003058_round52_persistent_onboarding.sql'],{encoding:'utf8'}),'','Round 42–43 production contracts and native evidence runners must be unchanged');
