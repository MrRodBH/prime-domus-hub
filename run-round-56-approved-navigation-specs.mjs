...
const {SUPER_NAVIGATION,workspaceContexts,workspaceItemActive,CONTEXTS}=await import(`data:text/javascript;base64,${Buffer.from(output.outputFiles[0].text).toString('base64')}`);
...
try{dom.window.fetch=()=>{throw Error('REMOTE_FORBIDDEN')};dom.window.eval(bundle.outputFiles[0].text);for(let i=0;i<100&&!dom.window.document.querySelector('nav');i++)await tick();
 ...
 assert.ok(d.querySelector('[aria-label="REAL ONE"]'));assert.equal(d.querySelector('img[alt="RM Prime"]'),null);
...
try{
 ...
 w.domainState={domains:[{id:'domain-fixture',normalizedHostname:'example.invalid',status:'active',enabled:true,hostnameKind:'canonical',generation:1,executionMode:'manual_assisted',lockVersion:1,registrableDomain:'example.invalid',metadata:{required_dns_plan:{hostname:'example.invalid',generation:1,recordType:'CNAME',targetHostname:'sites.delivery.invalid'}}}],challenges:{}};
 ...
 const button=text=>[...d.querySelectorAll('button')].find(x=>x.textContent===text);
 async function until(predicate){for(let i=0;i<150&&!predicate();i++)await tick();assert.ok(predicate())}
 await until(()=>button('Remover'));assert.ok(d.body.textContent.includes('sites.delivery.invalid'));assert.ok(d.body.textContent.includes('Configure este registro'));button('Remover').click();await until(()=>d.querySelector('[role="alertdialog"]'));assert.equal(w.domainCalls.length,0);button('Manter domínio').click();await until(()=>!d.querySelector('[role="alertdialog"]'));assert.equal(w.domainCalls.length,0);
